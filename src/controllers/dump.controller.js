...
/**
 * Core brain dump processing logic — shared by the HTTP endpoint and the Notion inbox poller.
 * Returns { saved, pendingId, aiResponse }.
 */
async function processDump(text) {
  ...
  return { saved, pendingId, aiResponse };
}
...
async function dump(req, res, next) {
  try {
    ...
    res.json(success(result, 'Brain dump processed'));
  } catch (err) {
    next(err);
  }
}
...
module.exports = { dump, followup, processDump };
